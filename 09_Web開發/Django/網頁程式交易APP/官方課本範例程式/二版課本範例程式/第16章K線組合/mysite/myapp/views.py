	...
	if(stock_id == "all"):
		...
		for table in tables:
			if(len(table) == 10 and table[:6] == 'stock_' and table[-4:].isdigit()):
				stkno = table[-4:]
				result = result.append(E_16_1_main(stkno, 'all', trend_type, trade_type), ignore_index=True)
		response = result
	else:
		response = E_16_1_main(stock_id, 'one', trend_type, trade_type)
