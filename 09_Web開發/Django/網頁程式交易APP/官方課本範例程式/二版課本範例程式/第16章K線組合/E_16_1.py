...
def trend_judgement(df, stkno, count, acmroi, winnum, winfact, trend_type, trade_type):
    print(trade_type)
    #MA_Type: 0=SMA, 1=EMA, 2=WMA, 3=DEMA, 4=TEMA, 5=TRIMA, 6=KAMA, 7=MAMA, 8=T3 (Default=SMA)
    close = np.array(df['close'], dtype=float)
    SMA = talib.SMA(close,3) #close 代進WMA方法做計算
    ...
    for i in range(row):
        ...
        if(trend_status!=None):
            #進行八種交易策略判斷
            trade_result = trade_judgement(df, i, trend_status, trade_type)
...
#交易策略
def trade_judgement(df, i, trend_status, trade_type):
    ...
    if(trend_status == True):
        #TWS
        if(trade == False) and ("TWS" in trade_type or trade_type == "all"):
            ans = 0
            ...
            if(ans == 4):
                trade = True
        #TIU
        if(trade == False) and ("TIU" in trade_type or trade_type == "all"):
            ans = 0
            ...
            if(ans == 4):
                trade = True
        #TOU
        if(trade == False) and ("TOU" in trade_type or trade_type == "all"):
            ans = 0
            ...
            if(ans == 4):
                trade = True
        #MS
        if(trade == False) and ("MS" in trade_type or trade_type == "all"):
            ans = 0
    ...
    ##下降趨勢
        ...
        #TBC
        if(trade == False) and ("TBC" in trade_type or trade_type == "all"):
            ans = 0
            if((df['close'].iloc[i]<df['open'].iloc[i]) and (df['close'].iloc[i-1]<df['open'].iloc[i-1]) and (df['close'].iloc[i-2]<df['open'].iloc[i-2])):
                ans +=1
            if((df['close'].iloc[i]<df['close'].iloc[i-1]) and (df['close'].iloc[i-1]<df['close'].iloc[i-2])):
            ...
            if(ans == 4):
                trade = True
        #TID
        if(trade == False) and ("TID" in trade_type or trade_type == "all"):
            ans = 0
            if((df['open'].iloc[i-2]<df['close'].iloc[i-2]) and (df['open'].iloc[i-2]<=df['open'].iloc[i-1]) and (df['open'].iloc[i-1]<df['close'].iloc[i-2])):
                ans +=1
            if((df['open'].iloc[i-2]<df['close'].iloc[i-1]) and (df['close'].iloc[i-1]<=df['close'].iloc[i-2])):
            ...
            if(ans == 4):
                trade = True
        #TOD
        if(trade == False) and ("TOD" in trade_type or trade_type == "all"):
            ans = 0
            if((df['open'].iloc[i-2]<df['close'].iloc[i-2]) and (df['close'].iloc[i-2]<df['open'].iloc[i-1])):
                ans +=1
            if((df['open'].iloc[i-2]<df['close'].iloc[i-2]) and (df['close'].iloc[i-1]<df['open'].iloc[i-2])):
            ...
            if(ans == 4):
                trade = True
        #ES
        if(trade == False) and ("ES" in trade_type or trade_type == "all"):
            ans = 0
            if(df['open'].iloc[i-2]<df['close'].iloc[i-2] and (abs(df['open'].iloc[i-1]-df['close'].iloc[i-1])>0)):
                ans +=1
            if((df['close'].iloc[i-2]<df['close'].iloc[i-1]) and (df['close'].iloc[i-2]<df['open'].iloc[i-1])):
...
def main(stkno, get_type, trend_type, trade_type): 
    df = select_db(stkno)    
    #進行趨勢判斷
    df, df_result = trend_judgement(df, stkno, count = 0, acmroi = 0, winnum = 0, winfact = 0, trend_type = trend_type, trade_type = trade_type)
    if(get_type == 'one'):
        showMAplot(df)
    print(df_result)
    return (df_result)